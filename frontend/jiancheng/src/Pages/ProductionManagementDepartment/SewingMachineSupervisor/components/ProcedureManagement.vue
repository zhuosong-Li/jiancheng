<template>
    <el-row :gutter="20">
        <el-col :span="24" :offset="0" style="font-size: xx-large; text-align: center; color: black">工序管理</el-col>
    </el-row>
    <el-row :gutter="20" style="margin-top: 20px">
        <el-col :span="4" :offset="0" style="white-space: nowrap;">
            搜索工序
            <el-input v-model="procedureSearch" placeholder="请输入订单号" clearable @keypress.enter="getProcedureData()"
...
export default {
    ...
    methods: {
        ...
        async getProcedureData() {
            let params = {"team": "针车预备"}
            let response = await axios.get(`${this.$apiBaseUrl}/production/getallprocedures`, { params })
            this.procedureData = response.data
            params = {"team": "针车"}
            response = await axios.get(`${this.$apiBaseUrl}/production/getallprocedures`, { params })
            this.procedureData = [...this.procedureData, ...response.data]
        },
        async editProcedure() {
            this.isEditing = true
        },
