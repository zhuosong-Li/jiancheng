<template>
  <el-container :direction="vertical">
      ...
      <el-main height="">
              ...
              <el-col :span="24" :offset="0">
                  <el-table :data="testTableFilterData" border style="height: 400px">
                      ...
                      <el-table-column
                          ...
                          align="center"
                      ></el-table-column>
                      <el-table-column prop="purchaseOrderId" label="采购订单编号" align="center">
                      </el-table-column>
                      <el-table-column
                          prop="status"
                          label="状态"
                      ...
                      <el-table-column label="操作" align="center" width="250">
                          <template #default="scope">
                              <el-button
                                  v-if="scope.row.status === '二次采购订单未填写'"
                                  type="primary"
                                  @click="handleGenerate(scope.row)"
                                  >填写</el-button
                              >
                              <el-button
                                  v-else-if="scope.row.status === '二次采购订单已下发'"
                                  type="primary"
                                  @click="openPreviewDialog(scope.row)"
                                  >查看</el-button
                              >
                              <div v-else-if="scope.row.status === '二次采购订单已保存'">
          ...
          <el-dialog
              :title="`二次BOM填写 ${newPurchaseOrderId}`"
              v-model="createVis"
              width="100%"
...
export default {
  ...
  methods: {
      ...
      async getAllShoeBomInfo() {
          const response = await this.$axios.get(
              `${this.$apiBaseUrl}/secondpurchase/getallboms?orderid=${this.orderId}`
          )
      ...
      async getOrderShoeBatchInfo(orderId, orderShoeId) {
          const response = await this.$axios.get(
              `${this.$apiBaseUrl}/order/getordershoesizesinfo`,
              {
                  params: {
                      orderid: orderId,
