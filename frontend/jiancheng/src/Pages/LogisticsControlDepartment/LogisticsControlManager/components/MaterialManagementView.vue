<template>
  ...
  <el-dialog
    title="添加新材料类型"
    v-model="isAddNewMaterialTypeDialogVisible"
    width="30%">
    <el-form :model="newMaterialTypeForm" label-width="80px">
      <el-form-item label="材料类型">
        <el-input v-model="newMaterialTypeForm.materialType" placeholder="请输入材料类型"></el-input>
      </el-form-item>
  ...
  <el-dialog title="批量添加新材料" v-model="newMaterialVis" width="60%">
    <el-table :data="newMaterialData" border>
      ...
      <el-table-column label="操作">
        <template #default="scope">
          <el-button type="danger" @click="deleteCurrentRow(scope.$index, newMaterialData)"
            >删除</el-button
          >
        </template></el-table-column
      >
