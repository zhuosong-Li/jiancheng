...
def dbcreateOrder(order_Rid, order_createTime, order_Customer):
    db_entity = Order(order_rid = order_Rid, order_createtime = order_createTime, order_customer = order_Customer)
    db.session.add(db_entity)
    db.session.commit()
    return True